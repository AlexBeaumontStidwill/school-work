...
void testJumble() {
	...
	JumblePuzzle *newPuzzle;
	//Test Accessors
	cout << "Creating new puzzle:" <<endl;
	string tester = "SUMMER";
	cout << "this is the hidden word: " << tester << endl;
	newPuzzle = new JumblePuzzle(tester, "hard");

	cout << "Word size should be: " << tester.length() << endl;
	cout << "Puzzle dimension: " << newPuzzle->getSize() << endl;
	cout << "The word should appear at row/column/direction \n" << newPuzzle->getRowPos() << "\n" << newPuzzle->getColPos() << "\n" << (int)newPuzzle->getDirection() << endl;
	cout << "Direction values: 0 is is bottom to top, 1 is left to right, 2 is top to bottom, 3 is right to left" << endl;

	showJumble(newPuzzle->getJumble(), newPuzzle->getSize());

	//test exceptions
	cout << "test exceptions" << endl;
	try {
		JumblePuzzle bad("notRight","answer");
	} catch(BadJumbleException &e) {
		cerr << e.what() << endl;
		cout << "Passed test for having wrong difficulty" << endl;
	}

	try{
		JumblePuzzle badAgain("thisStringIsObviouslyWayTooLong", "easy");
	} catch(BadJumbleException &e) {
		cerr << e.what() << endl;
		cout << "Passed test for having a longer than 10 character string" << endl;
	}

	try{
			JumblePuzzle shortBad("hi", "easy");
		} catch(BadJumbleException &e) {
			cerr << e.what() << endl;
			cout << "Passed test for having a smaller than 3 character string" << endl;
		}
